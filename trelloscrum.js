...
function getDaysFromMonday(monday, numWeeks) {
	daysArray = [];
	for (var i = 0; i < numWeeks * DAYS_IN_WEEK; i++) {
		daysArray.push(new Date(monday));
		monday.setDate(monday.getDate() + 1);
	}
	return daysArray;
}

function getIdealSeries(monday, numWeeks, points) {
	var days = getDaysFromMonday(monday, numWeeks);
	var dropRate = points / (numWeeks * DAYS_IN_WEEK);
	var pointsSeries = []
	for (var i = 0; i < days.length; i++) {
		pointsSeries.push(points - i * dropRate);
	}
	return days.map((e, i) => [e, pointsSeries[i]]);
}
...
function getCurrentBoard() {
	return window.location.href.match(/b\/(.*)\//)[1];
}

function getData(board) {
	return getSetting(board+'data');
}

function doAll() {
	var today = getToday();
	var previousMonday = getMonday(today);
	var numberOfWeeks = getNumberOfWeeks();
	refreshData();
	var currentBoard = getCurrentBoard();
	var data = getData(currentBoard);
	var initialPoints = getInitialPoints(data);
	var idealSeries = getIdealSeries(previousMonday, numberOfWeeks, initialPoints);
	composeGraph(data, idealSeries);
}
...
function showBurndown() {
	...
	var iFrameWrapper = $('<div/>', { id: "iframe-wrapper", style: 'padding:10px; padding-top: 13px;' });
...
function showChart() {
	...
	Highcharts.chart('iframe-wrapper', {
		title: {
